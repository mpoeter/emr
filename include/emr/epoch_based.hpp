...
namespace emr {
  ...
  class epoch_based<UpdateThreshold>::enable_concurrent_ptr :
    private detail::deletable_object_impl<T, Deleter>,
    private detail::tracked_object<epoch_based>
  {
  public:
  ...
  template <std::size_t UpdateThreshold>
  template <class T, class MarkedPtr>
  class epoch_based<UpdateThreshold>::guard_ptr : public detail::guard_ptr<T, MarkedPtr, guard_ptr<T, MarkedPtr>>
  {
...
#define EPOCH_BASED_IMPL
#include "epoch_based_impl.hpp"
#undef EPOCH_BASED_IMPL
