...
#include "detail/aligned_object.hpp"
#include <algorithm>
#include <new>
#include <vector>
...
namespace emr {
  ...
  template <typename Policy>
  template <class T, class MarkedPtr>
  void hazard_pointer<Policy>::guard_ptr<T, MarkedPtr>::acquire(concurrent_ptr<T>& p,
  ...
  struct alignas(64) basic_hp_thread_control_block :
    ...
    detail::aligned_object<basic_hp_thread_control_block<Policy, Derived>>
  {
  ...
  private:
    struct alignas(64) hazard_pointer_block : detail::aligned_object<hazard_pointer_block>
    {
  ...
  template <typename Policy>
  struct alignas(64) hazard_pointer<Policy>::thread_data : detail::aligned_object<thread_data>
  {
  ...
  template <typename Policy>
  emr::detail::allocation_tracker hazard_pointer<Policy>::allocation_tracker;
