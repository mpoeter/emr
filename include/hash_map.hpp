...
template <class Key, class Value, class Reclaimer, size_t Buckets>
class hash_map
{
public:
  struct node;
...
private:
  ...
  bool find(Key key, concurrent_ptr& head, find_info& info, detail::backoff& backoff);
};

template <class Key, class Value,class Reclaimer, size_t Buckets>
hash_map<Key, Value, Reclaimer, Buckets>::~hash_map()
{
...
template <class Key, class Value,class Reclaimer, size_t Buckets>
bool hash_map<Key, Value, Reclaimer, Buckets>::find(Key key, concurrent_ptr& head,
  find_info& info, detail::backoff& backoff)
{
retry:
  info.prev = &head;
...
template <class Key, class Value,class Reclaimer, size_t Buckets>
auto hash_map<Key, Value, Reclaimer, Buckets>::search(Key key) -> guard_ptr
{
  find_info info;
...
template <class Key, class Value,class Reclaimer, size_t Buckets>
bool hash_map<Key, Value, Reclaimer, Buckets>::insert(Key key, Value value)
{
  guard_ptr dummy;
...
template <class Key, class Value,class Reclaimer, size_t Buckets>
bool hash_map<Key, Value, Reclaimer, Buckets>::insert(Key key, Value value, guard_ptr& entry)
{
...
template <class Key, class Value,class Reclaimer, size_t Buckets>
bool hash_map<Key, Value, Reclaimer, Buckets>::remove(Key key)
{
  detail::backoff backoff;
  find_info info;
  auto& bucket = buckets[std::hash<Key>{}(key) % Buckets];
