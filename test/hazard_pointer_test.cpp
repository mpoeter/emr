...
struct my_static_hazard_pointer_policy : emr::static_hazard_pointer_policy<2>
{
...
struct my_dynamic_hazard_pointer_policy : emr::dynamic_hazard_pointer_policy<2>
{
  // we are redifining this method in our own policy to enforce the
  // immediate reclamation of nodes.
  static constexpr size_t retired_nodes_threshold() { return 0; }
...
{
  using HP = emr::hazard_pointer<Policy>;
...
TYPED_TEST(HazardPointer, acquire_guard_acquires_pointer)
{
  ...
  typename concurrent_ptr::guard_ptr gp = emr::acquire_guard(foo_ptr);
...
TYPED_TEST(HazardPointer, guard_ptr_protects_the_same_object_via_different_base_classes)
{
  ...
  static Foo* ptr; // variable has to exist even after test finishes
