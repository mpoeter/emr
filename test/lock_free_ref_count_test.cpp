...
struct Foo : emr::lock_free_ref_count<>::enable_concurrent_ptr<Foo, 2>
{
  Foo** instance;
...
template <typename T> using concurrent_ptr = emr::lock_free_ref_count<>::concurrent_ptr<T>;
...
{
  struct Dummy : emr::lock_free_ref_count<>::enable_concurrent_ptr<Dummy> {};
